...
function submitOrder(event) {
    ...
    // Добавление данных заказа на канвас (необязательно)
    ctx.font = '20px Arial';
    ctx.fillStyle = 'black';
    ctx.fillText(`Имя: ${name}`, 10, mannequinCanvas.height - 60);
    ctx.fillText(`Размер: ${size}`, 10, mannequinCanvas.height - 40);
    ctx.fillText(`Количество: ${quantity}`, 10, mannequinCanvas.height - 20);
    ctx.fillText(`Примечания: ${notes}`, 10, mannequinCanvas.height);

    // Сохранение состояния после добавления текста
    saveState();
    ...
    // Сброс дополнительных данных на канвасе (если необходимо)
    // ctx.clearRect(0, mannequinCanvas.height - 60, 800, 60);
}
